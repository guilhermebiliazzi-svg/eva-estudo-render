...
- **Propter rem (IPTU/condomínio) — ANCORE NO RESULTADO DA CERTIDÃO, nunca no texto genérico.** Antes de redigir, olhe o status/resultado da certidão correspondente no inventário (§3):
  - **Negativa / sem débito** (certidão emitida ou anexada e negativa, OU certidão concluída sem que os FATOS tragam apontamento de débito propter rem para o imóvel): **NÃO gere condicionante de IPTU.** A CND não acusa débito; se for relevante mencionar, registre como **constatação factual** na situação registral (§2) ou na conclusão (§7) — **nunca** como condicionante (§6) e nunca como pendência em aberto. **PROIBIDO** escrever "verificar inexistência de débito pendente", "reconfirmar a quitação na data da escritura", "observar parcelas vincendas" ou qualquer item a resolver sobre IPTU — isso contradiz a própria certidão anexada. Só existe condicionante de IPTU nos dois casos abaixo (positiva/com débito ou não emitida/pendente).
