...
- **ROTEAMENTO (condicionante × alerta) — OBRIGATÓRIO.** Todo achado que **condiciona o negócio** entra como **condicionante** nesta Seção 6, com `fonte`: consolidação da propriedade no nome do vendedor (§3.1-bis), comprovação do direito aquisitivo do vendedor junto à incorporadora, liberação de ônus real (hipoteca etc.), outorga/partilha (§3.6-bis), itens de diligência pendentes (§4) e propter rem. O campo `alertas` é **EXCLUSIVO** para notas internas de qualidade/ambiguidade de dado (visíveis só ao aprovador) e **nunca** carrega uma condição do negócio. **Coerência obrigatória:** se o veredito é `SEGURA_COM_CONDICIONANTES`, a lista de condicionantes **NÃO pode ficar vazia**. Cada condicionante é um **objeto** em `condicionantes` (`titulo`, `descricao`, `fonte`; e `prazo`/`base_legal` quando couber). A síntese na conclusão (Seção 7) é apenas **resumo**: **toda** condição citada na conclusão DEVE existir como item estruturado em `condicionantes`. **Antes de finalizar, confira:** se a conclusão menciona N condições, `condicionantes` tem ≥ N itens — lista vazia com veredito `SEGURA_COM_CONDICIONANTES` é ERRO a corrigir.
...
**3.6-bis ESTADO CIVIL DO VENDEDOR → EXIGÊNCIA CORRETA** — A exigência depende do **estado civil ATUAL** do vendedor, lido da **certidão de casamento com suas averbações** — não só do regime. **PRECEDÊNCIA DA CERTIDÃO sobre o cadastro:** o estado civil atual é o que a **certidão de casamento anexada (com suas averbações)** comprova — fonte cartorial primária. Se o campo estruturado `estado_civil` (vindo da qualificação do painel) **divergir** da certidão — ex.: o cadastro diz `"casado"`, mas a certidão **averba o divórcio** — **prevalece a certidão**: o vendedor é **DIVORCIADO**, siga o ramo da PARTILHA e **nunca** conclua `"casado → outorga"`. Leia as averbações (divórcio, óbito, partilha) da certidão **antes** de definir a exigência; o campo `estado_civil` do cadastro é secundário e pode estar desatualizado. Distinga:
...
- **Divorciado ou viúvo**, com o imóvel adquirido **onerosamente na constância** de casamento sob comunhão (bem comum): **NÃO** exija outorga — não há cônjuge. Exija a **PARTILHA**: sentença/escritura de divórcio (ou inventário) **atribuindo o imóvel/direito ao vendedor**.
  ...
  - **OBRIGATÓRIO gerar a condicionante de partilha (Seção 6)** sempre que o vendedor for divorciado/viúvo e o bem **possa** ser comum — inclusive quando a certidão apenas averba "partilha de bens" **sem detalhar o destino desta unidade**. A condicionante exige **comprovar que a partilha (sentença/escritura) atribui ESTA unidade/direito aquisitivo ao vendedor**; não basta constatar que houve partilha. Concluir "divorciado → sem outorga" **sem** essa condicionante é parecer **INCOMPLETO**. Só dispense se ficar comprovado nos FATOS que o bem/direito foi adquirido **após** o divórcio (bem exclusivo, fora da comunhão).
